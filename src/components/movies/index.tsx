...
import { moreMovies as movies } from '../../constants/data';
...
export const Movies = () => {
    ...
    const filteredMovies = selectedCategories.includes("All") || selectedCategories.length === 0
        ? movies
        : movies.filter((movie) =>
            selectedCategories.some((cat) =>
                movie.category.toLowerCase().includes(cat.toLowerCase())
            )
          );
    ...
    return (
        <MoviesSection id="movies">
            <SectionHeader
                ...
                withOutline={true}
                withBorder={false}
                onSeeMore={() => console.log('See More Movies clicked')}
            />
